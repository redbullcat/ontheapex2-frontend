...
function computeAverageRuns(
  ...
  const stints = computeCarStints(laps).filter(
    (s) =>
      s.laps.length >= minStintLength &&
      activeClasses.has(s.class ?? 'Unknown') &&
      activeCars.has(s.car_number),
  )

  const byCarAndPosition = new Map<string, Map<number, number[]>>()
  for (const stint of stints) {
    let byPosition = byCarAndPosition.get(stint.car_number)
    if (!byPosition) {
      byPosition = new Map()
      byCarAndPosition.set(stint.car_number, byPosition)
    }
  ...
  for (const stint of stints) if (!teamByCar.has(stint.car_number)) teamByCar.set(stint.car_number, stint.team)

  const out: CarAverageRun[] = []
  for (const [car, byPosition] of byCarAndPosition) {
    const averaged = [...byPosition.entries()]
      .map(([lapInStint, times]) => ({ lapInStint, avgTime: d3.mean(times) ?? 0 }))
      .sort((a, b) => a.lapInStint - b.lapInStint)

    // Top-N% fastest filter: keep only the fastest N% of position-averages
    // for this car (by time, not by position), same trimming rule as the
    // Streamlit reference — a car with a handful of slow outlier positions
    // (traffic, yellow flags) can hide them without losing the rest of the
    // run's shape.
    const keepCount = topPercent <= 0 ? 0 : Math.max(1, Math.ceil((averaged.length * topPercent) / 100))
    const keepPositions = new Set(
      [...averaged].sort((a, b) => a.avgTime - b.avgTime).slice(0, keepCount).map((p) => p.lapInStint),
    )
    const points = averaged.filter((p) => keepPositions.has(p.lapInStint))
    if (points.length === 0) continue
    out.push({ car_number: car, team: teamByCar.get(car) ?? null, points })
  }
  return out
}
...
export function AverageLongRunChart({
  laps,
  forcedWidth,
  onRendered,
}: {
  ...
  const [minStintInput, setMinStintInput] = useState(String(DEFAULT_MIN_STINT))
  ...
  const minStintLength = Math.max(1, Number(minStintInput) || DEFAULT_MIN_STINT)
  const topPercent = Math.max(0, Math.min(100, Number(topPercentInput) || 0))

  const runs = useMemo(
    () => computeAverageRuns(laps, activeClasses, activeCars, minStintLength, topPercent),
    [laps, activeClasses, activeCars, minStintLength, topPercent],
  )

  const strokeColor = useCallback((run: { team: string | null }) => getTeamColor(run.team), [])
  const pathsSelRef = useRef<d3.Selection<SVGPathElement, CarAverageRun, SVGGElement, unknown> | null>(null)
  ...
  useEffect(() => {
    ...
    const minTime = d3.min(runs, (r) => d3.min(r.points, (p) => p.avgTime)) ?? 0
    const maxTime = d3.max(runs, (r) => d3.max(r.points, (p) => p.avgTime)) ?? 1
    ...
    const paths = g
      .append('g')
      .selectAll<SVGPathElement, CarAverageRun>('path')
      .data(runs)
      .join('path')
      .attr('fill', 'none')
    ...
    overlay
      .on('mousemove', (event: MouseEvent) => {
        ...
        let nearest: { car: string; team: string | null; avgTime: number; lapInStint: number } | null = null
        let nearestDist = Infinity
        for (const run of runs) {
          for (const p of run.points) {
            const d = Math.hypot(x(p.lapInStint) - mx, y(p.avgTime) - my)
            if (d < nearestDist) {
              nearestDist = d
              nearest = { car: run.car_number, team: run.team, avgTime: p.avgTime, lapInStint: p.lapInStint }
            }
          }
        }
        if (!nearest) return
    ...
    <div className="viz-root long-run-chart" ref={containerRef}>
      ...
      <CollapsibleFilters
        actions={
          <ChartExportButtons
            svgRef={svgRef}
            filename="average_long_run_pace"
            renderChart={(w, onReady) => (
              <AverageLongRunChart laps={laps} forcedWidth={w} onRendered={onReady} />
            )}
          />
        }
      >
      ...
      {runs.length === 0 ? (
        ...
        <p className="hint">
          Average lap time by position within a stint, across every stint of at least {minStintLength} laps.
        </p>
      )}
      <svg ref={svgRef} />
      {hover && (
        <div className="tooltip" style={{ left: hover.x, top: hover.y }}>
          ...
          <div>
            Lap {hover.lapInStint} of stint · avg {formatSeconds(hover.avgTime)}
          </div>
        </div>
      )}
    </div>
