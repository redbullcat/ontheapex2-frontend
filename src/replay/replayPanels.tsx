...
function LeaderboardPanel({ ctx, onRowClick }: { ctx: ReplayPanelContext; onRowClick?: (car: string) => void }) {
...
export function renderReplayPanel(panel: PanelInstance, ctx: ReplayPanelContext, onRowClick?: (car: string) => void) {
  switch (panel.kind) {
    case 'leaderboard':
      return <LeaderboardPanel ctx={ctx} onRowClick={onRowClick} />
    case 'race-log':
      return (
        <RaceLogPanel
    ...
    case 'gap-evolution':
      return <ReplayTrendChart data={ctx.data} mode="gap" currentLap={ctx.leaderLap} title="Gap evolution" />
    case 'lap-position':
      return <ReplayTrendChart data={ctx.data} mode="position" currentLap={ctx.leaderLap} title="Lap-by-lap position" />
    case 'circle-of-doom': {
    ...
    case 'car-pace':
      return panel.carNumber ? <PaceChart laps={carLapsFor(ctx, panel.carNumber)} hideCarFilter /> : null
    case 'car-stints':
      return panel.carNumber ? <CarStintTable laps={carLapsFor(ctx, panel.carNumber)} /> : null
    case 'car-pit-stops':
      return panel.carNumber ? <PitTimeChart laps={carLapsFor(ctx, panel.carNumber)} /> : null
